...
impl CopilotCodeVerification {
    ...
    fn render_prompting_modal(
        ...
        v_flex()
            ...
            .child(
                Label::new("使用 Copilot 编辑预测需要在 GitHub 上订阅。").color(Color::Muted),
            )
    ...
    fn render_enabled_modal(cx: &mut Context<Self>) -> impl Element {
        v_flex()
            ...
            .child(
                Label::new("一切就绪，可以开始使用 Copilot 编辑预测了。").color(Color::Muted),
            )
            .child(
...
impl ConfigurationView {
    ...
    fn render_for_edit_prediction(&self) -> impl IntoElement {
        ...
        let no_status_label = "Copilot 编辑预测需要有效的 GitHub Copilot 订阅。请确保已配置 Copilot 编辑预测后重试，或使用其他编辑预测提供商。".into();
