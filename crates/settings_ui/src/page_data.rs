    ...
    fn general_settings_section(_cx: &App) -> Vec<SettingsPageItem> {
        vec![
            ...
            SettingsPageItem::SettingItem(SettingItem {
                title: "无障碍模式",
                description: "针对屏幕阅读器等辅助技术优化 Zed 界面。启用后，原本折叠的控件将保持展开并可通过键盘访问。",
                field: Box::new(SettingField {
                    organization_override: None,
...
fn appearance_page() -> SettingsPage {
    ...
    fn agent_panel_font_section() -> [SettingsPageItem; 5] {
        [
            ...
            SettingsPageItem::SettingItem(SettingItem {
                title: "UI 字体族",
                description: "代理面板中代理回复文本的字体族。未设置时使用常规界面字体族。",
                field: Box::new(SettingField {
                    organization_override: None,
            ...
            SettingsPageItem::SettingItem(SettingItem {
                title: "缓冲区字体族",
                description: "代理面板中用户消息的字体族。未设置时使用常规缓冲区字体族。",
                field: Box::new(SettingField {
                    organization_override: None,
    ...
    fn which_key_section() -> [SettingsPageItem; 3] {
        [
            ...
            SettingsPageItem::SettingItem(SettingItem {
                ...
                description: "当多键快捷键等待后续按键时，显示包含匹配绑定的 Which-key 菜单。待输入按键指示器仍会显示，但会禁用它的绑定预览弹窗。",
                field: Box::new(SettingField {
                    organization_override: None,
    ...
    fn gutter_section() -> [SettingsPageItem; 10] {
        [
            ...
            SettingsPageItem::DynamicItem(DynamicItem {
                discriminant: SettingItem {
                    title: "Git 边栏宽度",
                    description: "边栏中 Git 差异指示器的宽度（像素）。未设置时，宽度随缓冲区字体大小缩放。",
                    field: Box::new(SettingField {
                        organization_override: None,
...
fn search_and_files_page() -> SettingsPage {
    ...
    fn file_scan_section() -> [SettingsPageItem; 7] {
        [
            ...
            SettingsPageItem::SettingItem(SettingItem {
                title: "文件扫描深度",
                description: "在 Git 仓库之外预先索引的最大目录深度；达到或超过该深度的目录内容按需索引。根目录浅于该深度的仓库始终完整索引。若项目本身不以 Git 仓库为根，则根文件夹直接子级的仓库会立即启用 Git 功能，更深的仓库在首次使用时启用。0 表示不限制并立即激活所有 Git 仓库。",
                field: Box::new(SettingField {
                    organization_override: None,
...
fn window_and_layout_page() -> SettingsPage {
    fn status_bar_section() -> [SettingsPageItem; 12] {
        [
            ...
            SettingsPageItem::SettingItem(SettingItem {
                title: "待输入按键指示器",
                description: "当多键快捷键等待后续按键时，在状态栏显示倒计时指示器。启用 Which-key 菜单时会禁用它的绑定预览弹窗。",
                field: Box::new(SettingField {
                    organization_override: None,
    ...
    fn window_section() -> [SettingsPageItem; 6] {
        [
            ...
            SettingsPageItem::SettingItem(SettingItem {
                title: "标题格式",
                description: "窗口标题模板。可用变量包括 `${projectName}`、`${fileName}`、`${filePath}`、`${relativePath}`、`${fileStem}`、`${remoteName}`、`${remoteHost}`、`${appName}`、`${branch}` 和 `${separator}`。当相邻变量为空时会省略 `${separator}`，但会保留字面文本。协作指示器存在时会追加到渲染后的模板之后。如果模板渲染结果为空，则使用默认模板。",
                field: Box::new(SettingField {
                    organization_override: None,
            ...
            SettingsPageItem::SettingItem(SettingItem {
                title: "标题分隔符",
                description: "窗口标题格式中用于替换 `${separator}` 的字符串。需要包含周围的空格时，请直接写在值里。",
                field: Box::new(SettingField {
                    organization_override: None,
            ...
            SettingsPageItem::SettingItem(SettingItem {
                title: "全屏模式",
                description: "（仅 macOS）切换全屏操作进入的全屏模式。",
                field: Box::new(SettingField {
                    organization_override: None,
    ...
    fn project_panel_section() -> [SettingsPageItem; 30] {
        [
            ...
            SettingsPageItem::DynamicItem(DynamicItem {
                discriminant: SettingItem {
                    title: "项目面板标题工具提示延迟",
                    description: "项目面板标题工具提示出现前的延迟时间，以毫秒为单位。",
                    field: Box::new(SettingField {
                        organization_override: None,
                    ...
                    .map(|variant| match variant {
                        ...
                        settings::ProjectPanelTitleTooltipDelayDiscriminants::Custom => {
                            vec![SettingItem {
                                files: USER,
                                title: "自定义延迟",
                                description: "项目面板标题工具提示的延迟时间，以毫秒为单位。",
                                field: Box::new(SettingField {
                                    organization_override: None,
                                    json_path: Some("project_panel.title_tooltip_delay"),
                                    pick: |settings_content| match settings_content
                ...
                fields: dynamic_variants::<settings::WorkingDirectory>()
                    ...
                    .map(|variant| match variant {
                        ...
                        settings::WorkingDirectoryDiscriminants::Always => vec![SettingItem {
                            files: USER | PROJECT,
                            title: "目录",
                            description: "要使用的目录路径（会进行 shell 展开）。",
                            field: Box::new(SettingField {
                                organization_override: None,
...
fn version_control_page() -> SettingsPage {
    ...
    fn git_hunks_section() -> [SettingsPageItem; 5] {
        [
            ...
            SettingsPageItem::SettingItem(SettingItem {
                title: "差异基准",
                description: "Git 功能是相对 HEAD（未提交更改）还是相对默认分支（当前分支上的全部更改）显示变更。",
                field: Box::new(SettingField {
                    organization_override: None,
    ...
    fn general_section() -> [SettingsPageItem; 6] {
        [
            ...
            SettingsPageItem::SubPageLink(SubPageLink {
                ...
                description: Some("查看、添加和移除通过 Agent Client Protocol 连接的代理。".into()),
                search_aliases: &[
                    "acp",
