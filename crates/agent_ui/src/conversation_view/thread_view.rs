...
fn zh_effort_name(name: SharedString) -> SharedString {
    match name.as_ref() {
        "Low" => "低".into(),
        "Medium" => "中".into(),
        "High" => "高".into(),
...
impl ThreadView {
    ...
    fn render_thinking_control(&self, cx: &mut Context<Self>) -> Option<AnyElement> {
        ...
        let (tooltip_label, icon, color) = if thinking {
            ("禁用思考模式", IconName::ThinkingMode, Color::Muted)
        } else {
            (
                "启用思考模式",
    ...
    fn render_effort_selector(
        ...
        let label = selected
            ...
            .map_or("选择强度".into(), |effort| zh_effort_name(effort.name));
    ...
    fn render_send_button(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        if self.is_loading_contents {
            ...
            IconButton::new("send-message", send_icon)
                ...
                .tooltip(move |_window, cx| {
                    if is_editor_empty && !is_generating {
                        Tooltip::for_action("输入内容后发送", &Chat, cx)
                                    ...
                                    v_flex()
                                        ...
                                        .when(project_rules_count > 0, move |this| {
                                            ...
                                            this.child(
                                                Button::new(
                                                    "open-project-rules",
                                                    format!("{} 条项目规则", project_rules_count),
                                                )
                                                .end_icon(
                                                    Icon::new(IconName::ArrowUpRight)
                                                        .color(Color::Muted)
...
impl ThreadView {
                            ...
                            h_flex()
                                ...
                                .child(
                                    Button::new("restore-checkpoint", "恢复检查点")
                                        ...
                                        .label_size(LabelSize::XSmall)
                                        .color(Color::Muted)
                                        .tooltip(Tooltip::text("将项目中的所有文件恢复到此对话节点时的内容。"))
                                        .on_click(cx.listener(move |this, _, _window, cx| {
    ...
    fn render_thread_controls(
        ...
        let open_as_markdown = IconButton::new("open-as-markdown", IconName::FileMarkdown)
            ...
            .tooltip(Tooltip::text("以 Markdown 打开会话"))
            .on_click(cx.listener(move |this, _, window, cx| {
                if let Some(workspace) = this.workspace.upgrade() {
        ...
        let last_turn_tokens_label = last_turn_clock
            ...
            .then(|| {
                self.turn_fields
                    ...
                    .map(|tokens| {
                        Label::new(format!("{} 令牌", crate::humanize_token_count(tokens)))
                            .size(LabelSize::Small)
                            .color(Color::Muted)
                    })
            })
        ...
        right_click_menu(format!("agent_context_menu-{}", entry_ix))
            ...
            .menu(move |window, cx| {
                ...
                ContextMenu::build(window, cx, move |menu, _, cx| {
                    ...
                    let copy_this_agent_response = ContextMenuEntry::new("复制这条代理回复")
                        .handler({
                            let entity = entity.clone();
                            move |_, cx| {
    ...
    fn render_multi_root_callout(&self, cx: &mut Context<Self>) -> Option<Callout> {
        ...
        Some(
            Callout::new()
                ...
                .description(format!("当前默认只会在“{}”中运行。", active_dir))
                .border_position(self.callout_border_position())
                .dismiss_action(
